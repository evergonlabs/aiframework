...
## Code Style
- Enforced by linter: $(echo "$m" | jq -r '.quality.linter.tool // "not configured"')
- Formatted by: $(echo "$m" | jq -r '.quality.formatter.tool // "not configured"')

## Testing
- Framework: $(echo "$m" | jq -r '.quality.test_framework.tool // "not configured"')
- Run: \`${test_cmd}\`
