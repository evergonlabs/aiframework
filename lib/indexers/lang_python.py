...
def _find_parent_class(content: str, func_start: int) -> str | None:
    ...
    for m in _CLASS_RE.finditer(before):
        pass  # iterate to the last class before this position
    # Walk backwards through class matches to find the enclosing one
    best: str | None = None
    for m in _CLASS_RE.finditer(before):
        best = m.group(1)
    return best
...
def parse_python(filepath: str, content: str) -> dict:
    """Parse a Python file and return symbols, imports, and exports.
    ...
    for m in _IMPORT_RE.finditer(content):
        raw = m.group(1)
        # Skip "from X import Y" lines (already handled above)
