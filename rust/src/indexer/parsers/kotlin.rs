...
static RE_CLASS: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^\s*(?:data\s+|sealed\s+|abstract\s+|open\s+)?class\s+(\w+)").unwrap()
});
...
static RE_FUN: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^\s*(?:private\s+|internal\s+|protected\s+)?(?:suspend\s+)?fun\s+(\w+)\s*\(([^)]*)\)").unwrap()
});
...
pub fn parse(content: &str, _filepath: &str) -> (Vec<Symbol>, Vec<String>, Vec<String>) {
    ...
    for (i, line) in content.lines().enumerate() {
        ...
        if let Some(caps) = RE_OBJECT.captures(trimmed) {
            let name = caps[1].to_string();
            symbols.push(Symbol {
                name: name.clone(),
                kind: "class".into(),
        ...
        if let Some(caps) = RE_FUN.captures(trimmed) {
            let name = caps[1].to_string();
            let params = caps[2].to_string();
            let kind = if current_class.is_some() { "method" } else { "function" };
            let visibility = if trimmed.starts_with("private") || trimmed.starts_with("internal") {
                "private"
            } else {
                "public"
            };
            ...
            symbols.push(Symbol {
                ...
                kind: kind.into(),
                line: lineno,
                signature: format!("fun {name}({params})"),
                docstring: String::new(),
